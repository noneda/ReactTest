		    ...
		    <div class="list">
		      <table border="0" cellspacing="0">
		        ...
		        <tr ng-repeat="item in list | orderBy: '-checked':true">
		          <td>
                    <input ng-show="!item.checked" ng-checked="item.checked" ng-click="add(item, list)" type='checkbox' checklist-value="item.checked"/></td>
		          <td ng-class="{selected: item.checked==true}"></td>
		          <td ng-class="{selected: item.checked==true}" class="left"></td>
		          <td ng-class="{selected: item.checked==true}" class="center"></td>
		          <td ng-class="{selected: item.checked==true}"></td>
		          <td><i class="icon-cross delete" ng-click="remove(item)"></i></td>
		        </tr>
		      </table>
		      <div ng-hide="list.length" class="center">Your shopping list is empty.</div>
		      <div class="cost cf">
		        <span class="left">Total Cost:</span>
		        <span class="right" ng-class="{total: length!==0}"></span>
		      </div>
